...
Last week, my tech-savvy architect wanted access to a production OAuth client secret for debugging a customer data integrity issue. Simple enough, right? Well, the secret was stored in LastPass and, shocker, he didn't have access to our team's shared vault.

_Teams Chat - 2:34 AM_

<div style="background:rgb(230, 228, 228); padding: 1rem; border-radius: 0.5rem; margin: 1rem 0;">
...
Ok fine. Maybe it wasn't 2 AM, but you get the point. Obviously he needs to send a request to get LastPass access on the corporate system, but that can take a day or two. In the meantime, should I send the plaintext password and have the SREs come for my first-born? Surely there is a more secure way...

## Initial Ideas

Oh so, assuming he really needed the secret *now*. I conjured up some other options:

1. **Encrypted zip files**\
The problem with this approach is that it becomes a Matroska doll of sending secrets, because now how do you share the password to the zip file securely?
...
Minutes pass - my architect is ready to send off the obligatory Frodo gif.
...
But hold on, walking through the basic requirements - I need a way to send a secret in such a way that only the recipient can decrypt it. Of course..

## Enter Public-Key Cryptography

This is exactly what public-key cryptography was designed for! I could encrypt the secret using his public key, and only he would be able to decrypt it with his private key.
...
I initially considered using OpenSSL since it's ubiquitous. However, I had run into version mismatch issues the other day when trying to use it to decrypt a file. We'd need to exchange public keys in a PEM format, and the commands aren't particularly user-friendly.

## GPG: Keeping it real

GPG (GNU Privacy Guard) turned out to be a great fit for this scenario. As a fan of GNU tools, I appreciate that GPG embodies the free software philosophy. While OpenSSL is also open source, GPG feels more purpose-built for exactly this use case: secure communication between people, with user-friendly key management built right in. Let's give it a go.
...
Within a few minutes, we had securely shared the production secret. No LastPass delays, no security red alerts, and no plaintext secrets sitting in chat logs.
...
The good news too is that now I have his public key, there is 0 overhead to encrypt other secrets across as well. Go GNU!
